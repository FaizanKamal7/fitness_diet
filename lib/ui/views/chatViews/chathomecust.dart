...
class ChatHomeCust extends StatefulWidget {
  final String currentUserId;

  ChatHomeCust({Key key, @required this.currentUserId}) : super(key: key);

  @override
  State createState() => ChatHomeCustState(currentUserId: currentUserId);
}

class ChatHomeCustState extends State<ChatHomeCust> {
  ChatHomeCustState({Key key, @required this.currentUserId});
  ...
  Widget buildItem(BuildContext context, ChatUser document) {
    // String user = await DatabaseService().checkUserID(document.data()['idUser']);

    if (document.idUser == currentUserId) {
      return Container();
    } else {
      return Container(
        child: FlatButton(
